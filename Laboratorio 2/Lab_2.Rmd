...
```{r}
planes_flights <- merge(flights, planes, by = "tailnum")
```
...
double_flights_airports <- merge(flights_airports, airports, by.x = "dest", by.y = "faa")
```
...
colnames(filtered_flights)[1] <- "dst"
filteredflights_airports <- inner_join(filtered_flights, airports, by = "dst")
filteredflights_airports
```
